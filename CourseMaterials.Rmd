...
```{r}
logs %>% group_by(Team) %>% summarize(average_3P_attempts = mean(Team3PA)) %>% arrange(desc(average_3P_attempts)) -> top_shooters
```
...
```{r}
big10cumulative <- big10 %>% group_by(Team) %>% mutate(fgs = cumsum(TeamFG))

msc <- big10cumulative %>% filter(Team == "Michigan State Spartans")
psc <- big10cumulative %>% filter(Team == "Penn State Nittany Lions")

```

```{r}
ggplot() + 
  geom_step(data=big10cumulative, aes(x=Date, y=fgs, group=Team), color="light grey") + 
  geom_step(data=msc, aes(x=Date, y=fgs, group=Team), color="dark green") +
  geom_step(data=psc, aes(x=Date, y=fgs, group=Team), color="dark blue")
```
...
ggplot() + 
  ...
  geom_step(data=msc, aes(x=Date, y=fgs, group=Team), color="dark green") +
  geom_step(data=psc, aes(x=Date, y=fgs, group=Team), color="dark blue") +
   labs(x="Date", y="Baskets made", title="A tale of two seasons", subtitle="Michigan State and Penn State went it different directions and sit at opposite ends of the Big Ten Tournament", caption="Source: Sports-Reference.com | By Matt Waite") 
```
...
ggplot() + 
  ...
  geom_step(data=msc, aes(x=Date, y=fgs, group=Team), color="dark green") +
  geom_step(data=psc, aes(x=Date, y=fgs, group=Team), color="dark blue") +
   labs(x="Date", y="Baskets made", title="A tale of two seasons", subtitle="Michigan State and Penn State sit at opposite ends of the Big Ten Tournament.", caption="Source: Sports-Reference.com | By Matt Waite") +
  theme(
...
ggplot() + 
  ...
  geom_step(data=msc, aes(x=Date, y=fgs, group=Team), color="dark green") +
  geom_step(data=psc, aes(x=Date, y=fgs, group=Team), color="dark blue") +
   labs(x="Team", y="Baskets made", title="A tale of two seasons", subtitle="Michigan State and Penn State sit at opposite ends of the Big Ten Tournament", caption="Source: Sports-Reference.com | By Matt Waite") +
  theme(
    plot.title = element_text(size = 16, face = "bold"),
